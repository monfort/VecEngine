// ==========================================================================
// 					Class Specification : CMRulerOrganizer
// ==========================================================================
...
to do with the appearance of the ruler bar. This class is called CMRulerBar.

CMRulerBar is derived from CWnd and can be used independently as a stand alone 
...
CMRulerBar itself is useful control but in most cases you would be intersted to use
...
horizontal and/or vertical ruler bars for any window. The CMRulerOrganizer class
provides that kind of functionality.

In order to provide ruler bars for any window you have to do the following:

1)	declare an object of CMRulerOrganizer class so the lifetime of such object would 
...
2)	Attach the window to CMRulerOrganizer object using Attach() function. In this 
	...
	int CMyView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
	{
		if (CMyView::OnCreate(lpCreateStruct) == -1)
			return -1;
		. . . . . . . . . . . . . . . 
		// atach view to the CMRulerOrganizer object and display 
...
After a window is attached to the CMRulerOrganizer object you can use a lot of
CMRulerOrganizer functions that would allow you to utilize this class functionality
to the full extent.
...
4)	Direct access to the underlying CMRulerBar objects:

	GetHorzRulerBar();
	GetVertRulerBar();

5)	Set of wrapper functions that call corresponding CMRulerBar functions:
...
attached to the corresponding CMRulerOrganizer object. For example: 
...
The sample that demonstrate how to use CMRulerOrganizer class is called RulerBar 
...
Source code files:
	"MRulerOrganizer.cpp"
	"MHookWnd.cpp"			-	implementation of the CMHookWnd class 
								from which the CMRulerOrganizer class is derived

*/



#ifndef _MRULERORGANIZER_H__
#define _MRULERORGANIZER_H__

#if _MSC_VER >= 1000
#pragma once
#endif // _MSC_VER >= 1000

#define WM_RULER_EVENTS WM_USER
#define WM_RULER_NCMOUSEMOVE       (WM_NCMOUSEMOVE     + WM_RULER_EVENTS)
#define WM_RULER_NCLBUTTONDOWN     (WM_NCLBUTTONDOWN   + WM_RULER_EVENTS)
#define WM_RULER_NCLBUTTONUP       (WM_NCLBUTTONUP     + WM_RULER_EVENTS)
#define WM_RULER_NCLBUTTONDBLCLK   (WM_NCLBUTTONDBLCLK + WM_RULER_EVENTS)
#define WM_RULER_NCRBUTTONDOWN     (WM_NCRBUTTONDOWN   + WM_RULER_EVENTS)
#define WM_RULER_NCRBUTTONUP       (WM_NCRBUTTONUP     + WM_RULER_EVENTS)
#define WM_RULER_NCRBUTTONDBLCLK   (WM_NCRBUTTONDBLCLK + WM_RULER_EVENTS)
#define WM_RULER_NCMBUTTONDOWN     (WM_NCMBUTTONDOWN   + WM_RULER_EVENTS)
#define WM_RULER_NCMBUTTONUP       (WM_NCMBUTTONUP     + WM_RULER_EVENTS)
#define WM_RULER_NCMBUTTONDBLCLK   (WM_NCMBUTTONDBLCLK + WM_RULER_EVENTS)

#include "MHookWnd.h"


const int MRB_HORZHEIGHT=18;
const int MRB_VERTWIDTH=18;

const int MRBO_BORDER=2;

// timer for checking scroll position
const UINT IDT_MRBO_CHECK_SCROLLPOS=243;
// delay between two checks
const UINT MRBO_CHECK_SCROLLPOS_DELAY=100;


#define RULLER_HORIZONTAL 0
#define RULLER_VERTICAL 1
#define RULLER_BLANK 2

/////////////////////////////////////////////////////////////////////////////
// CMRulerBar window

class CMRulerBar : public CWnd
{
// Construction
public:
	// --- In  :
...
public:
	...
	void Scroll(double dPos, BOOL bRedraw=TRUE);
	void Offset(double dPos, BOOL bRedraw=TRUE);
	...
	inline double GetCurrentPos() const { return m_dPos; }
	...
	void Zoom(double dZoom, BOOL bRedraw=TRUE);

	// --- In :		
	// --- Out :
	// --- Returns:	current zoom level
	// --- Effect : retrieves the current zoom level (1% - original size)
	inline double GetZoomLevel() const { return m_dZoom; }
...
class CMRulerOrganizer : public CMHookWnd
{
protected:
	...
	CMRulerBar m_horzRulerBar;
	// vert ruler bar
	CMRulerBar m_vertRulerBar;
...
public:
	...
	inline void RedrawAttached(BOOL bRecalcSize=FALSE) {
		if(bRecalcSize)
		{
			m_pWndHooked->SetWindowPos(NULL,0,0,0,0,
				SWP_FRAMECHANGED|SWP_NOMOVE|SWP_NOSIZE|SWP_NOZORDER);
		}
		m_pWndHooked->RedrawWindow(NULL,NULL,RDW_FRAME|RDW_INVALIDATE|RDW_UPDATENOW);
	}
	...
	inline CMRulerBar* GetHorzRulerBar() { return &m_horzRulerBar; }

	// --- In  :	
	// --- Out : 
	// --- Returns:	pointer to the vertical ruler bar object
	// --- Effect : retrieves the pointer to the vertical ruler bar object
	inline CMRulerBar* GetVertRulerBar() { return &m_vertRulerBar; }
	...
	//				scale in pixels
	inline void ScrollRuler(double dPos, BOOL bHorizontal, BOOL bRedraw=TRUE) 
	{
		if(bHorizontal)
			m_horzRulerBar.Scroll(dPos,FALSE);
		else
			m_vertRulerBar.Scroll(dPos,FALSE);
		if(bRedraw)
			RedrawAttached();
	}

   inline void OffsetRuler(double dPos, BOOL bHorizontal, BOOL bRedraw=TRUE) 
	{
		if(bHorizontal)
			m_horzRulerBar.Offset(dPos,FALSE);
		else
			m_vertRulerBar.Offset(dPos,FALSE);
		if(bRedraw)
			RedrawAttached();
	}
	...
	inline double GetRulerCurrentPos(BOOL bHorizontal) const 
	{
		if(bHorizontal)
	...
	inline void ZoomRuler(double dZoom, BOOL bHorizontal, BOOL bRedraw=TRUE) 
	{
		if(bHorizontal)
		{
			m_horzRulerBar.Zoom(dZoom,FALSE);
			m_horzRulerBar.Scroll(m_pWndHooked->GetScrollPos(SB_HORZ)*100/dZoom,FALSE);
		}
		else
		{
			m_vertRulerBar.Zoom(dZoom,FALSE);
			m_vertRulerBar.Scroll(m_pWndHooked->GetScrollPos(SB_VERT)*100/dZoom,FALSE);
		}
		if(bRedraw)
			RedrawAttached();
	}
	...
	inline double GetRulerZoomLevel(BOOL bHorizontal) const 
	{
		if(bHorizontal)
	...
	void CheckScrollPos(BOOL bDoRedraw = TRUE);
